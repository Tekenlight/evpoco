...
static struct ev_rwlock_s sg_dbtmm_lock;
...
void init_locks_if_not_done()
{
	...
	if (l_i_d == 0) {
		if (atomic_compare_exchange_strong(&sg_lock_init_done, &l_i_d, 1)) {
			...
			EV_RW_LOCK_S_INIT(sg_dbtmm_lock);
		}
	}
	return;
}
...
{
	if (l) ev_rwlock_wrlock(&sg_dbtmm_lock);
	else ev_rwlock_wrunlock(&sg_dbtmm_lock);
...
{
	if (l) ev_rwlock_rdlock(&sg_dbtmm_lock);
	else ev_rwlock_rdunlock(&sg_dbtmm_lock);
