...
static int open_connection_initiate(lua_State *L)
{
    ...
    //DEBUGPOINT("TAKING A CONNECTION TO [%s] {\n", dbname);
...
static int connection_status(lua_State *L)
{
    connection_t *conn = (connection_t *)luaL_checkudata(L, 1, EV_POSTGRES_CONNECTION);
    if (conn->conn_in_error == 1) {
        lua_pushstring(L, "CONN_IN_ERROR == 1");
        lua_pushboolean(L, 0);
    }
    else {
        switch (PQtransactionStatus(conn->pg_conn)) {
            case PQTRANS_ACTIVE:
                lua_pushstring(L, "PQTRANS_ACTIVE");
                lua_pushboolean(L, 0);
                break;
            case PQTRANS_INTRANS:
                lua_pushstring(L, "PQTRANS_INTRANS");
                lua_pushboolean(L, 0);
                break;
            case PQTRANS_INERROR:
                lua_pushstring(L, "PQTRANS_INERROR");
                lua_pushboolean(L, 0);
                break;
            case PQTRANS_UNKNOWN:
                //DEBUGPOINT("ALL NOT OK\n");
                lua_pushstring(L, "PQTRANS_UNKNOWN");
                lua_pushboolean(L, 0);
                break;
            case PQTRANS_IDLE:
                lua_pushstring(L, "PQTRANS_IDLE");
                lua_pushboolean(L, 1);
                break;
            default:
                lua_pushstring(L, "ALL_OK");
                lua_pushboolean(L, 1);
                break;
        }
    }
...
static int connection_close(lua_State *L)
{
    ...
    //DEBUGPOINT("Reached:RETURNING A CONN of [%s] }\n", conn->s_dbname->c_str());
    if (conn->conn_in_error == 1) {
        ...
        switch (PQtransactionStatus(conn->pg_conn)) {
            case PQTRANS_ACTIVE:
                DEBUGPOINT("PQTRANS_ACTIVE\n");
                return orig_connection_close(L);
            case PQTRANS_INTRANS:
                DEBUGPOINT("PQTRANS_INTRANS\n");
                return orig_connection_close(L);
            case PQTRANS_INERROR:
                DEBUGPOINT("PQTRANS_INERROR\n");
                return orig_connection_close(L);
            case PQTRANS_UNKNOWN:
                DEBUGPOINT("PQTRANS_UNKNOWN\n");
                DEBUGPOINT("ALL NOT OK\n");
                return orig_connection_close(L);
            case PQTRANS_IDLE:
                //DEBUGPOINT("ALL OK\n");
