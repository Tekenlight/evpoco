...
static int connection_new(lua_State *L) {
	...
	lua_pushnil(L);
	lua_pushfstring(L, EV_SQL_ERR_CONNECTION_FAILED, mysql_error(conn->mysql));
	return 2;
    }
    ...
    luaL_getmetatable(L, EV_MYSQL_CONNECTION);
...
static int connection_prepare(lua_State *L) {
    ...
    if (conn->mysql) {
	return ev_mysql_statement_create(L, conn, luaL_checkstring(L, 2)); 
    }
